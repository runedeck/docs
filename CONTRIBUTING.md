...
```sh
git clone https://github.com/runedeck/docs.git
cd docs
...
A same-repository pull request opened ready for review starts its first cascade automatically; every later round is summoned by label. Bare `review` runs the cascade: cursor, then macroscope, then the adjudicating correctness lane. `review:runeseer`, `review:macroscope`, and `review:autofix` summon a single lane. A round consumes its labels when it ends, and every round is visible in the pull request timeline. A fork pull request starts no lane and cannot satisfy the required correctness check: the owner reviews it directly and merges it through the owner-veto path.
