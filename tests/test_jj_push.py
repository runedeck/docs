...
class BookmarkPushTests(unittest.TestCase):
    ...
    def test_checkpoint_and_synthetic_refs_stay_outside_validation(self):
        backend = self.jj("git", "root", cwd=self.workspace)
        unrelated = (
        ...
        result = self.push("--bookmark", BOOKMARK)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        record = self.assert_hook(previous=ZERO)
        for name in unrelated:
            self.assertNotIn(f"refs/heads/{name}", record["refs"])
            self.assertIsNone(self.remote_target(name))
